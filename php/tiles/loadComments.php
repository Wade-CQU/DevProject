...
// Get comments:
$sql = "SELECT c.id as cid, c.text as comment, u.userName as name, u.role as role, postDate FROM comment c LEFT JOIN (SELECT id, CONCAT(firstName,' ',lastName) as userName, role FROM user) u ON c.userId = u.id WHERE tileId = ? ORDER BY c.id DESC;";
